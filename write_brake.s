...
;This subroutine is used to write to LCD as "Brake On"
...
;Save the blank codes to memory address
				MOV			R0,#0x00
				STRB		R0,[R1],#1
;Save the "B" codes to memory adress				
				MOV			R0,#0x7f
				STRB		R0,[R1],#1
				MOV			R0,#0x49
				STRB		R0,[R1],#1
...
;Save the blank codes to memory address
				MOV			R0,#0x00
				STRB		R0,[R1],#1				
;Save the "K" codes to memory adress				
				MOV			R0,#0x7f
				STRB		R0,[R1],#1
				MOV			R0,#0x08
				STRB		R0,[R1],#1
...
Data_LCD		
				...
				LDR			R4,=0x20000745	;which shows the end of memory of this word "BRAKE ON"
...
Write_brake_LCD										
wait_until_transmit_FIFO_not_full_Brake
				LDR			R1,=SSI0_SR		;Status Register
				LDR			R0,[R1]
				AND			R0,#0x02		;TNF flag is obtained.
				CMP			R0,#0x02		
				BNE			wait_until_transmit_FIFO_not_full_Brake ;If TNF=0, then transmit buffer is full
				BEQ			send_data_Brake		;If TNF=1, then send a new data.

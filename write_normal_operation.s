...
normal_address			EQU			0x20000700	;It is used to store "=>Normal Op." hexa codes to write it on LCD
...
;LABEL			DIRECTIVE		VALUE		COMMENT
				...
				EXPORT		write_normal_operation
				
write_normal_operation	PROC	
				PUSH		{R0-R12}


;This subroutine is used to write to LCD as "Normal Op"


;Save the "=" codes to memory adress				
				LDR			R1,=normal_address
				MOV			R0,#0x14
				STRB		R0,[R1],#1
				MOV			R0,#0x14
				STRB		R0,[R1],#1
...
;Save the blank codes to memory address
				MOV			R0,#0x00
				STRB		R0,[R1],#1
;Save the "N" codes to memory adress				
				MOV			R0,#0x7f
				STRB		R0,[R1],#1
				MOV			R0,#0x04
				STRB		R0,[R1],#1
...
;Save the blank codes to memory address
				MOV			R0,#0x00
				STRB		R0,[R1],#1
;Save the "o" codes to memory adress				
				MOV			R0,#0x38
				STRB		R0,[R1],#1
				MOV			R0,#0x44
				STRB		R0,[R1],#1
...
;Save the blank codes to memory address
				MOV			R0,#0x00
				STRB		R0,[R1],#1
;Save the "l" codes to memory adress				
				MOV			R0,#0x00
				STRB		R0,[R1],#1
				MOV			R0,#0x41
				STRB		R0,[R1],#1
...
;Save the blank codes to memory address
				MOV			R0,#0x00
				STRB		R0,[R1]
;Save the "P" codes to memory adress				
				MOV			R0,#0x7f
				STRB		R0,[R1],#1
				MOV			R0,#0x09
				STRB		R0,[R1],#1				
...
Data_LCD		
				...
				LDR			R3,=normal_address
				LDR			R4,=0x20000745	;which shows the end of memory of this word "=>Normal Op."
				
check_if_word_finishes				
				CMP			R3,R4
				BNE			Write_normal_op_LCD
				BEQ			exit
				
;This part writes to LCD: "Meas(mm):"				
Write_normal_op_LCD										
wait_until_transmit_FIFO_not_full_Norm
				LDR			R1,=SSI0_SR		;Status Register
				LDR			R0,[R1]
				AND			R0,#0x02		;TNF flag is obtained.
				CMP			R0,#0x02		
				BNE			wait_until_transmit_FIFO_not_full_Norm ;If TNF=0, then transmit buffer is full
				BEQ			send_data_Norm		;If TNF=1, then send a new data.
